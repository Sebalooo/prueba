...
<div class="py-12">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="cursor-pointer px-6 py-3" wire:click="order('rut')">
                                        RUT
                                        @if ($sort == 'rut')
                                            @if ($direction == 'asc')
                                                <i class="fas fa-sort-alpha-up-alt float-right mt-1"></i>
                                    ...
                                    <th scope="col" class="cursor-pointer px-6 py-3" wire:click="order('telefono')">
                                        TELEFONO
                                        @if ($sort == 'telefono')
                                            @if ($direction == 'asc')
                                                <i class="fas fa-sort-alpha-up-alt float-right mt-1"></i>
                                        ...
                                        <td class="px-6 py-4">
                                            {{$item->telefono}}
                                               ...
                                               //Botones para editar y eliminar
                                                ...
                                                <td class="px-6 py-4 ">
                                                    {{-- @livewire('inventario.editar-inventario', ['inventario' => $inv], key($inv->id)) --}}
                                                    <a class="btn btn-red" wire:click="$emit('borrarProve',{{$item->id}})">
    ...
    <x-jet-dialog-modal  wire:model="open_edit">
        ...
        <x-slot name="content">
            <div class="mb-4">
                ...
                <x-jet-input wire:model="prove.nombre" type="text" class="w-full" />
                <x-jet-input-error for='prove.nombre'/>
            </div>
            <div class="mb-4">
                <x-jet-label value="RUT" />
                <x-jet-input wire:model="prove.rut" type="text" class="w-full" />

                <x-jet-input-error for='prove.rut'/>
            </div>
            <div class="mb-4">
                <x-jet-label value="Telefono" />
                <x-jet-input wire:model="prove.telefono" type="text" class="w-full" />

                <x-jet-input-error for='prove.telefono'/>
            </div>
        </x-slot>
        <x-slot name="footer">
    ...
    <script src="{{ asset('vendor/sw2/sweetalert2.all.min.js') }}"></script>
        <script>
            Livewire.on('borrarProve',itemId => {
            Swal.fire({
